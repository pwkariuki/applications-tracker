...
function Stat({ label, value, accent }: { label: string; value: string; accent?: string }) {
  return (
    <div className="rounded-lg bg-muted/50 p-4">
      <div className={`text-2xl font-medium leading-none ${accent ?? ""}`}>{value}</div>
...
export default function StatsRow({ applications }: StatsRowProps) {
  ...
  return (
    <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
      ...
      <Stat label="Active" value={String(stats.active)} accent="text-blue-600 dark:text-blue-400" />
      <Stat label="Offers" value={String(stats.offers)} accent="text-emerald-600 dark:text-emerald-400" />
